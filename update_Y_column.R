...
# to update the st_close_price_pred column
#
...
full_stock_x <- model.matrix(~ . - 1, stock_prices[, -6])  # transform the variables to numeric
full_stock_y <- stock_prices[, 6]
xgb_full_stock <- xgb.DMatrix(data = full_stock_x, label=full_stock_y)
...
predict_xgb <- predict(model_xgb, newdata = xgb_full_stock)
...
full_metrics_xgb <- data.frame(mm_name = 'XGBoost',
                              mm_date = Sys.Date(),
...
dbWriteTable(con, "model_metrics", full_metrics_xgb, append = TRUE, row.names = FALSE)

full_predict_xgb <- data.frame(st_ticker = stock_prices$st_ticker,
...
for (i in 1:nrow(full_predict_xgb)) {
  query <- sprintf(
    "UPDATE stock SET st_close_price_pred = %f WHERE st_ticker = '%s' and st_date = '%s'",
    full_predict_xgb$st_close_price_pred[i],
    full_predict_xgb$st_ticker[i],
    format(full_predict_xgb$st_date[i], '%Y-%m-%d')
  )
  dbExecute(con, query)
}
...
if (evaluate_xgb["RMSE"] < model_metrics$mm_rmse && 
  ...
  saveRDS(model_xgb, "C:/Users/elain/OneDrive/Documents/NSCC/Term_4/DBAS3090 - Applied Data Analytics/repo/Project/outputs/full_final_model_xgb.rds")
...
write.csv(full_predict_xgb, "C:/Users/elain/OneDrive/Documents/NSCC/Term_4/DBAS3090 - Applied Data Analytics/repo/Project/outputs/full_predict_xgb.csv", row.names = FALSE)
write.csv(full_metrics_xgb, "C:/Users/elain/OneDrive/Documents/NSCC/Term_4/DBAS3090 - Applied Data Analytics/repo/Project/outputs/full_metrics_xgb.csv", row.names = FALSE)
##============================================================================

# # predict all data in stock table
# full_x <- model.matrix(~ . - 1, stock[, -6])
# xgb_full <- xgb.DMatrix(data = full_x)
# 
# # Use the model to predict all stock data on stock table
# predict_xgb_full <- predict(model_xgb, xgb_full)
# 
# # save the prediction into a dataframe
# save_pred_price <- data.frame(
#   st_ticker = stock$st_ticker,
#   st_date = stock$st_date,
#   st_close_price_pred = predict_xgb_full
# )
# 
# # save the predictions in database
# for (i in 1:nrow(save_pred_price)) {
#   query <- sprintf(
#     "UPDATE stock SET st_close_price_pred = %f WHERE st_ticker = '%s' and st_date = '%s'",
#     save_pred_price$st_close_price_pred[i],
#     save_pred_price$st_ticker[i],
#     format(save_pred_price$st_date[i], '%Y-%m-%d')
#   )
#   dbExecute(con, query)
# }